...
from .designteam_design_config import load_designteam_design_context_block
from .designteam_design_phases import DESIGN_PHASES_TABLE_MARKDOWN, designteam_phase_label_for_iteration
...
_DESIGNTEAM_AGENT_CAPABILITIES: dict[str, str] = {
...
_DESIGNTEAM_AGENT_ALIASES: dict[str, str] = {
...
_DESIGNTEAM_USAGE = (
    ...
    "Available agents: "
    + ", ".join(f"`{k}`" for k in sorted(_DESIGNTEAM_AGENT_CAPABILITIES))
)


def _parse_designteam_args(tail: str) -> tuple[str | None, str, bool, int, str]:
    ...
    if selected_agent and selected_agent not in _DESIGNTEAM_AGENT_CAPABILITIES:
        selected_agent = _DESIGNTEAM_AGENT_ALIASES.get(selected_agent, selected_agent)
    if selected_agent and selected_agent not in _DESIGNTEAM_AGENT_CAPABILITIES:
        return (
            None,
            "",
...
def _build_designteam_prompt(
    ...
    *,
    ...
    roster_lines = [
        f"- `{name}`: {desc}" for name, desc in sorted(_DESIGNTEAM_AGENT_CAPABILITIES.items())
    ]
    roster_block = "\n".join(roster_lines)
    if selected_agent:
        mode_block = (
    ...
    design_cfg_block = (design_yaml_context or "").strip() or "- (no `.claw/design/designteam/*.yaml` loaded)"
    ...
    return (
        "You are running clawcode built-in `/designteam` as the primary orchestrator agent.\n"
...
async def handle_builtin_slash(
    ...
    *,
    ...
    if head == "designteam":
        selected_agent, request, deep_loop, max_iters_ds, err = _parse_designteam_args(tail)
        if err:
            return BuiltinSlashOutcome(kind="assistant_message", assistant_text=err)
        svc_ds = LearningService(settings)
        selected_roles_ds = (
            [selected_agent] if selected_agent else sorted(_DESIGNTEAM_AGENT_CAPABILITIES.keys())
        )
        tecaps_ds = svc_ds.retrieve_team_capsules_for_clawteam(
            problem_type="general",
            participants=selected_roles_ds,
            team="active-workspace",
            top_k=3,
        )
        tecap_context_ds = [
                    ...
                    len(
                        {
                            (p.agent_id or "").strip().lower()
                            for p in x.participants
                            if (p.agent_id or "").strip().lower() in {r.lower() for r in selected_roles_ds}
                        }
                    )
                    / max(1, len(selected_roles_ds)),
                    6,
                ),
            }
            for x in tecaps_ds
        ]
        role_ecap_context_ds = svc_ds.retrieve_role_ecaps_for_clawteam(
            problem_type="general", participants=selected_roles_ds, top_k=1
        )
        design_yaml_context = load_designteam_design_context_block(root, selected_roles_ds)
        cfg_ds = svc_ds.get_designteam_deeploop_config()
        domain_ds, _ = resolve_domain(None, {"query": request, "session_title": getattr(ctx, "session_title", "")})
        ts_ms_ds = int(time.time() * 1000)
        trace_id_ds = f"trace-designteam-{ts_ms_ds}"
        cycle_id_ds = f"cycle-designteam-{ts_ms_ds}"
        designteam_deeploop_meta: dict[str, Any] | None = None
        if deep_loop:
            emit_ops_event(
                "designteam_deeploop_started",
                {
                    "trace_id": trace_id_ds,
                    "cycle_id": cycle_id_ds,
                    "policy_id": "designteam-deeploop-v1",
                    "domain": domain_ds,
                    "experiment_id": "",
                    "team_id": "active-workspace",
                    "problem_type": "general",
                    "selected_roles": selected_roles_ds,
                    "tecap_id": tecaps_ds[0].tecap_id if tecaps_ds else "",
                },
            )
            role_ecap_map_ds = {
                r: str((role_ecap_context_ds.get(r) or {}).get("ecap_id", "") or "").strip()
                for r in selected_roles_ds
            }
            role_ecap_map_ds = {k: v for k, v in role_ecap_map_ds.items() if v}
            designteam_deeploop_meta = {
                "trace_id": trace_id_ds,
                "cycle_id": cycle_id_ds,
                "tecap_id": str(tecaps_ds[0].tecap_id if tecaps_ds else ""),
                "role_ecap_map": role_ecap_map_ds,
                "policy_id": "designteam-deeploop-v1",
                "domain": domain_ds,
                "experiment_id": "",
            }
        prompt_ds = _build_designteam_prompt(
            request,
            selected_agent,
            deep_loop=deep_loop,
            max_iters=max_iters_ds,
            iter_idx=1,
            tecap_context=tecap_context_ds,
            role_ecap_context=role_ecap_context_ds,
            design_yaml_context=design_yaml_context,
            deeploop_thresholds={
                "min_gap_delta": cfg_ds.get("min_gap_delta", 0.05),
                "convergence_rounds": cfg_ds.get("convergence_rounds", 2),
                "handoff_target": cfg_ds.get("handoff_target", 0.85),
            },
        )
        return BuiltinSlashOutcome(
            kind="agent_prompt",
            agent_user_text=prompt_ds,
            designteam_deeploop_meta=designteam_deeploop_meta,
        )
    ...
    if head == "designteam-deeploop-finalize":
        sid = (getattr(ctx, "session_id", "") or "").strip()
        if not sid:
            return BuiltinSlashOutcome(
                kind="assistant_message",
        ...
        meta = designteam_deeploop_get_pending(sid)
        if not meta:
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text=(
                    "No pending designteam deep loop metadata for this session.\n\n"
                    "Run **`/designteam --deep_loop …`** first, then either wait for automatic finalize "
                    "when the assistant includes **`DEEP_LOOP_WRITEBACK_JSON:`**, or paste the assistant "
                    "block after **`/designteam-deeploop-finalize`** (or omit the tail to use the last "
                    "assistant message from history)."
                ),
            )
        output_text = (tail or "").strip()
        ...
        if not tecap_id:
            return BuiltinSlashOutcome(
                ...
                assistant_text=(
                    "Pending deep loop metadata has no `tecap_id` (no team capsule matched). "
                    "Create or import a TECAP, then run **`/designteam --deep_loop …`** again."
                ),
            )
        role_ecap_raw = meta.get("role_ecap_map")
        role_ecap_map: dict[str, str] = (
        ...
        if not res.get("skipped"):
            designteam_deeploop_clear_pending(sid)
        body = (
            "### designteam deep loop finalize\n\n"
            f"```json\n{json.dumps(res, ensure_ascii=False, indent=2)}\n```\n"
        )
        return BuiltinSlashOutcome(kind="assistant_message", assistant_text=body)
    ...
    if head == "ui-style":
        from .ui_style import load_ui_catalog, ui_style_auto_pick_rubric_text

        sub = tail.strip().split()[0].lower() if tail.strip() else ""
        wd_val = (settings.working_directory or ".").strip() or "."
        cli_dir = getattr(settings, "cli_launch_directory", None) or None
        catalog = load_ui_catalog(wd_val, cli_launch_directory=cli_dir)
        slug_map = {s.slug: s for s in catalog}

        current_mode = settings.ui_style_mode or "off"
        current_slug = ctx.ui_style_selected or ""

        if sub in ("off",):
            try:
                settings.ui_style_mode = "off"
            except Exception:
                pass
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text="UI style mode set to **off** — no style prefix will be injected.\n",
            )

        if sub in ("on",):
            try:
                settings.ui_style_mode = "on"
            except Exception:
                pass
            n_styles = len(catalog)
            hint = f" ({n_styles} style(s) loaded)" if n_styles else " (no catalog found — place one at `.claw/design/UI/catalog.json`)"
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text=f"UI style mode set to **on**{hint}.\n",
            )

        if sub in ("auto",):
            try:
                settings.ui_style_mode = "on"
                if hasattr(settings, "ui_style_selected"):
                    settings.ui_style_selected = ""
            except Exception:
                pass
            n_styles = len(catalog)
            hint = f" ({n_styles} style(s) in catalog)" if n_styles else " (no catalog found)"
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text=f"UI style set to **auto**{hint}. Each prompt will auto-select the best matching style.\n",
            )

        if sub in ("hybrid",):
            try:
                settings.ui_style_mode = "hybrid"
                if hasattr(settings, "ui_style_selected"):
                    settings.ui_style_selected = ""
            except Exception:
                pass
            ...
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text=f"UI style set to **hybrid**{hint}. Each prompt will auto-select the best style and attach a top-10 delegation menu for the LLM to override if needed.\n",
            )

        if sub.startswith("set "):
            target_slug = tail.strip()[4:].strip()
            if not target_slug:
                return BuiltinSlashOutcome(
                    kind="assistant_message",
                    assistant_text="Usage: `/ui-style set <slug>`.\nUse `/ui-style list` to see available styles.\n",
                )
            if target_slug not in slug_map:
                valid = ", ".join(f"`{s.slug}`" for s in catalog[:20]) if catalog else "(none)"
                return BuiltinSlashOutcome(
                    kind="assistant_message",
                    assistant_text=f"Unknown style `{target_slug}`. Available: {valid}.\n",
                )
            try:
                settings.ui_style_mode = "on"
                if hasattr(settings, "ui_style_selected"):
                    settings.ui_style_selected = target_slug
            except Exception:
                pass
            entry = slug_map[target_slug]
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text=f"UI style locked to **{target_slug}** ({entry.title}).\n",
            )
        ...
        if sub in ("list", "ls"):
            if not catalog:
                return BuiltinSlashOutcome(
                    kind="assistant_message",
                    assistant_text="No UI styles found. Place a catalog at `.claw/design/UI/catalog.json`.\n",
                )
            lines = ["# Available UI Styles\n"]
            for s in catalog:
                active = " **← active**" if s.slug == current_slug and current_mode != "off" else ""
                tag_str = ", ".join(s.tags[:4]) if s.tags else ""
                tag_hint = f" — {tag_str}" if tag_str else ""
                lines.append(f"- **{s.slug}**: {s.title}{tag_hint}{active}\n")
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text="".join(lines),
            )

        if sub in ("why",):
            reason = ctx.ui_style_reason or "(no selection reason recorded this session)"
            rubric = ui_style_auto_pick_rubric_text()
            top = ctx.ui_style_top_candidates or []
            source = ctx.ui_style_source or current_mode
            lines = [
                "# UI Style Selection Reason\n",
                f"- **mode:** {current_mode}\n",
                f"- **source:** {source}\n",
                f"- **selected:** {current_slug or '(none)'}\n",
                f"- **reason:** {reason}\n",
            ]
            if top:
                lines.append(f"- **top candidates:** {', '.join(top[:10])}\n")
            lines.append(f"\n{rubric}\n")
            return BuiltinSlashOutcome(
                kind="assistant_message",
                assistant_text="".join(lines),
        ...
        mode_label = current_mode if current_mode != "off" else "off"
        slug_label = current_slug or "(auto)" if current_mode != "off" else "—"
        help_text = (
            "# UI Style Status\n\n"
            f"- **mode:** {mode_label}\n"
            f"- **selected:** {slug_label}\n"
            f"- **catalog styles:** {len(catalog)}\n\n"
            "**Subcommands:**\n"
            "- `/ui-style on` — enable UI style injection (top-1 auto-select)\n"
            "- `/ui-style hybrid` — auto-select + top-10 delegation menu for LLM override\n"
            "- `/ui-style off` — disable UI style injection\n"
            "- `/ui-style auto` — alias for `on` (auto-select best style per prompt)\n"
            "- `/ui-style set <slug>` — lock a specific style\n"
            "- `/ui-style list` — show available styles\n"
            "- `/ui-style why` — explain current selection\n"
        )
        return BuiltinSlashOutcome(
            kind="assistant_message",
